// Cross-checks the TPresortEncoder against YDB's own CompareTypedCellVectors.
//
// The eq-height histogram relies on memcmp over TPresortEncoder-encoded keys producing
// the same order as YDB's cell comparison.  This test generates random typed tuples,
// encodes them with TPresortEncoder, and asserts that memcmp on the encoded keys agrees
// with CompareTypedCellVectors on the original cells for every pair.
//
// We are not testing *our* codec any more; we are pinning the claim that presort's order
// equals YDB's.
...
struct TTypedValue {
    ...
    TString Data; // raw cell bytes (native endianness for numerics, raw bytes for strings)
    ...
    NScheme::TTypeInfoOrder TypeInfoOrder() const {
        // Decimal is a parameterized type and cannot be constructed from TypeId alone.
...
// Build a TUnboxedValuePod from the raw cell bytes of a typed value.
// Presort's Encode() dispatches on EDataSlot and reads the value via Get<T>() for
// primitives or AsStringRef() for strings, so we must materialize the right kind of
// pod value.  For optionals, an empty pod (default-constructed) signals "no value";
// a present value is built from the payload and made optional.
TUnboxedValue MakePod(const TTypedValue& v) {
    ...
    switch (slot) {
        ...
        case EDataSlot::Yson:
        case EDataSlot::Uuid: {
            // String values need a TStringValue-backed pod so AsStringRef() works.
            // MakeString allocates via the UDF allocator (requires TScopedAlloc).
            pod = NMiniKQL::MakeString(TStringRef(v.Data.data(), v.Data.size()));
            break;
        }
        default:
            // Types not covered by presort (e.g. Tz*, Pg) are out of scope for the
            // histogram key encoder; the agreement test only covers key types.
            pod = NMiniKQL::MakeString(TStringRef(v.Data.data(), v.Data.size()));
            break;
    }
...
// Encode a typed tuple into a memcomparable byte key using TPresortEncoder.
// AddType is called once per element (matching the UDF's per-encoder type registration),
// then Start/Encode/Finish run per tuple.  Encode() handles the optional flag inline.
TString EncodeTuple(const TTypedTuple& t) {
    ...
    const TStringBuf buf = enc.Finish(); // borrowed -- copy before the encoder is reused
...
// Compare two typed tuples using YDB's own CompareTypedCellVectors.
...
// Helper: build a single-element tuple from raw bytes of a fixed-width type.
...
// Fixture providing a scoped UDF allocator so that MakeString() (used for
// string-typed TUnboxedValuePod) can allocate via the MKQL memory pool.
...
// Basic sanity: NULL sorts before present, matching CompareTypedCells.
...
// Signed integers: negative < zero < positive, matching YDB.
...
// Unsigned integers: 0 < 1 < 255 < 256, matching YDB.
...
// Floats: -inf < -1 < 0 < 1 < inf, matching YDB.
Y_UNIT_TEST(Floats) {
...
// -0.0 and +0.0 must encode to *identical* bytes.  CompareTypedCells reports them
// equal (va == vb), and presort's FP_ZERO code collapses both to a single code byte,
// so the encodings must be byte-for-byte equal.
Y_UNIT_TEST(MinusZeroVsPlusZero) {
    ...
    // YDB reports them equal.
    UNIT_ASSERT_VALUES_EQUAL(Sign(CompareWithYdb(a, b)), 0);
    // Presort must encode them identically.
...
// NaN is excluded from the agreement sweep: CompareTypedCells uses plain `<` for
// floats, which answers "greater" in both directions and is not transitive, so
// there is nothing to agree with.  This case pins presort's placement of NaN above
// +inf (EFPCode::Nan == 5 > EFPCode::PosInf == 4).
Y_UNIT_TEST(NanPlacement) {
    ...
    // Presort orders NaN above +inf.
    int encCmp = Sign(CompareEncoded(EncodeTuple(a), EncodeTuple(b)));
    UNIT_ASSERT_VALUES_EQUAL_C(encCmp, 1,
        "NaN must sort above +inf in presort, got enc=" << encCmp);
}

// Strings: lexicographic order, matching YDB's memcmp-based comparison.
// Covers the cases that break naive encodings: "b" > "aa", "a" < "ab", the empty
// string, and embedded NULs.
Y_UNIT_TEST(Strings) {
...
// Embedded NUL bytes: presort uses fixed-width 16-byte blocks with a trailing
// length byte, so 0x00 needs no escaping.  "a\0b" < "ab" must hold.
...
// The 15/16-character block boundary: a trailing length byte 0x0F meets the next
// block marker 0x1F.  A 15-byte string and a 16-byte string must compare correctly.
Y_UNIT_TEST(StringBlockBoundary) {
    ...
    // A 15-byte "a"*15 vs 15-byte "a"*14 + "b": the latter is larger.
    {
        TString s15a(15, 'a');
        TString s15b(14, 'a');
        s15b.append('b');
...
// Multi-column tuples: lexicographic across columns, matching YDB.
...
// Decimal: presort (NYql::NDecimal::Serialize) and YDB (pair<ui64,i64> high-first)
// reach the same ordering by different routes.  Include it deliberately; if YQL's
// special decimals (Inf/NaN/error) disagree, the supported key types should drop
// Decimal rather than relaxing this assertion.
Y_UNIT_TEST(Decimal) {
    // Build decimals as pair<ui64 low, i64 high> (YDB's on-disk form) and as
    // NYql::NDecimal::TInt128 (presort's input form) from the same logical value.
    auto makeDecimal = [](i64 high, ui64 low) -> TTypedValue {
        TTypedValue v;
        v.TypeId = NScheme::NTypeIds::Decimal;
        v.IsNull = false;
        // YDB cell layout: pair<ui64, i64> = {low, high}.
        std::pair<ui64, i64> p{low, high};
        v.Data = TString(reinterpret_cast<const char*>(&p), sizeof(p));
        return v;
    };

    // A few ordinary decimals in ascending order.
    ...
    for (size_t i = 0; i < tuples.size(); ++i) {
        for (size_t j = i + 1; j < tuples.size(); ++j) {
            ...
            // Only assert agreement for ordinary (non-special) decimals.  YQL's
            // Inf/NaN/error codes lie outside YDB's pair<ui64,i64> domain, so we
            // restrict the sweep to normal values.
            UNIT_ASSERT_VALUES_EQUAL_C(encCmp, ydbCmp,
...
// Randomized cross-check: generate random typed tuples and verify that
// the TPresortEncoder agrees with CompareTypedCellVectors for every pair.
Y_UNIT_TEST(RandomAgreement) {
    ...
    // Fixed column types for this run (as in real tables).  Includes optional
    // columns to exercise the inline has-value byte.
    struct TColDesc {
        NScheme::TTypeId TypeId;
        bool IsOptional;
    };
