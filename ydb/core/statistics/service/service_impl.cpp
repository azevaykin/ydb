            ...
            case EStatType::EQ_HEIGHT_HISTOGRAM:
                try {
                    ...
                    response.EqHeightHistogram.Data =
                        std::make_shared<TEqHeightHistogram>(msg->Data->data(), msg->Data->size());
                } catch (const std::exception& ex) {
                    // A blob written by a newer node (e.g. version 2) read by an
                    // older one after a rollback throws here; report failure
