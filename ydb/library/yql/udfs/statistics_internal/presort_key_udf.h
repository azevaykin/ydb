        ...
        if (!typesOnly) {
            ...
            if (argTupleInspector) {
                ...
                for (ui32 i = 0; i < argTupleInspector.GetElementsCount(); ++i) {
                    auto elemType = argTupleInspector.GetElementType(i);
                    TElementDesc desc;
                    auto optInspector =
                    ...
                    desc.Slot = NYql::NUdf::GetDataSlot(dataInspector.GetTypeId());
                    elements.push_back(desc);
                }
            } else {
                auto optInspector =
                    NYql::NUdf::TOptionalTypeInspector(*typeHelper, argType);
                auto innerType = argType;
                TElementDesc desc;
                if (optInspector) {
                    desc.IsOptional = true;
                    innerType = optInspector.GetItemType();
                }
                auto dataInspector =
                    NYql::NUdf::TDataTypeInspector(*typeHelper, innerType);
                if (!dataInspector) {
